...
const { INCORRECT_ERROR_CODE, NOT_FOUND_ERROR_CODE, DEFAULT__ERROR_CODE } = require('../app');

module.exports.getUsers = (req, res) => {
  User.find({})
    .then((user) => res.send({ data: user }))
    .catch(() => res.status(DEFAULT__ERROR_CODE).send({ message: 'Ошибка по умолчанию' }));
};
...
module.exports.getUserById = (req, res) => {
  User.findById(req.params.userId)
    ...
    .catch((err) => {
      ...
      return res.status(DEFAULT__ERROR_CODE).send({ message: 'Ошибка по умолчанию' });
    });
};
